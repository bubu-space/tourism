  ...
  <a-card :bordered="false" style="margin-bottom: 10px">
    <a-form ref="searchFormRef" layout="inline" :model="searchFormState">
      <a-form-item name="title" label="标题">
        <a-input v-model:value="searchFormState.title" placeholder="请输入标题" style="width: 240px;" />
      </a-form-item>
			<a-form-item>
				<a-space>
    ...
    >
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'category'">
          {{ $TOOL.dictTypeData('BIZ_RECOMMEND', record.category) }}
        </template>
...
<script setup>
    ...
    parameter.category = "PRODUCTION"
		return recommendApi.page(Object.assign(parameter, searchFormState.value)).then((res) => {
			return res
		})
	}
  ...
  // 删除
	const remove = (id) => {
		let params = [{id}]
		recommendApi.remove(params).then(() => {
			tableRef.value.refresh(true)
		})
	}
	// 批量删除
